...
const TaskDetails = () => {
  ...
  return (
    <div className=" w-full relative min-h-screen bg-slate-600">
      <div className=" max-w-[1300px] px-10 max-md:px-5 m-auto">
        <div>
          <TopNav title={"Task Details"} />

          <div className=" rounded-2xl bg-slate-700 max-w-[600px] m-auto mt-16 py-10 px-8 max-sm:p-5 text-white">
              ...
              <div className="max-sm:text-base font-semibold flex gap-4 items-center pt-4 pb-3">
                ...
                <div className="flex flex-wrap gap-3 max-sm:gap-2">
                  {details.catagory.map((val, index) => (
                    <p
                      className="text-left text-base bg-slate-600 border-slate-300 border-2 flex items-center gap-1 font-medium rounded-2xl px-3 py-1 max-sm:text-xs max-sm:py-0"
                      key={index}
                    >
